def isInput():
    ...
    return [int(i) for i in valueList]  
                                                                # Convert floating point value into integer by using inline for loop.

def main():
    FirstSide, SecondSide, ThirdSide = isInput()

    print(f"{FirstSide} {SecondSide} {ThirdSide}")              # Print Formated output.
                                                                # Output Number (Min to Max)
    a = FirstSide ** 2 + SecondSide ** 2 
    b = ThirdSide ** 2
                                                                # Set a, b variable
                                                                # Equilateral triangle
    if (FirstSide == SecondSide or SecondSide == ThirdSide):
